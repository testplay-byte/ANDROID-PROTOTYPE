# docs/navigation.md

> Index of everything in `docs/`. These are the reference documents that govern how we work.

---

## Files

| File                          | What it covers                                                      |
|-------------------------------|---------------------------------------------------------------------|
| `workflow.md`                 | Step-by-step process for creating, updating, and retiring a prototype. |
| `tech-stack.md`               | The allowed tech for prototypes and *why* each choice was made.      |
| `design-standards.md`         | Mobile UI/UX standards: spacing, type scale, color, touch targets, the phone frame. |
| `github-pages.md`             | How deployment works, how to find live URLs, how to troubleshoot.    |
| `notification-protocol.md`    | The ntfy.sh protocol: topic, colors, format, example commands.       |
| `git-conventions.md`          | Branch, commit message, and PR conventions.                         |

---

## Reading order for a new agent

1. [`../STARTUP.md`](../STARTUP.md)
2. [`../navigation.md`](../navigation.md)
3. This file.
4. [`workflow.md`](./workflow.md) — so you know how to make a prototype.
5. [`design-standards.md`](./design-standards.md) — so your prototype looks right.
6. [`notification-protocol.md`](./notification-protocol.md) — so you notify correctly.

---

*Last updated: repository initialization.*
